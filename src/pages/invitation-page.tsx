...
export function InvitationPage() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => { setSearchText(e.target.value); setShowDropdown(true) }
  const handleSelectGuest = (result: GuestSearchResult) => { setSelectedGuest(result); setFindView('table'); setShowDropdown(false) }
  const handleBackToSearch = () => { setFindView('search'); setSelectedGuest(null); setSearchText(''); setTimeout(() => inputRef.current?.focus(), 100) }
  ...
  return (
    <div className="gp-page" style={{ background: bg, color: text }}>
      <div className="gp-container">
        {logoUrl && <div className="gp-logo-wrapper"><img src={logoUrl} alt="Event logo" className="gp-logo" style={{ width: `${Math.min(logoSize, 500)}px`, maxWidth: '100%', height: 'auto', borderRadius: logoRounded ? '50%' : '0' }} /></div>}
        ...
        {subtitle && subtitle.trim() && <p className="gp-subtitle" style={{ fontFamily: getFontCss(subtitleFont), fontSize: `${settings.font_subtitle_size ?? 16}px`, color: settings.font_subtitle_color ?? text }}>{subtitle}</p>}
            ...
            {findView === 'search' && (
              <div className="gp-search-section">
                <div className="gp-autocomplete-wrapper" ref={searchWrapperRef}>
                  <input ref={inputRef} type="text" className="gp-autocomplete-input" placeholder="SEARCH YOUR NAME" value={searchText} onChange={handleSearchChange} onKeyDown={handleKeyDown} onFocus={() => { if (searchText.trim()) setShowDropdown(true) }} autoComplete="off" style={{ background: bg, borderColor: primary, color: text, borderRadius: radius }} />
                  {showDropdown && searchText.trim() && (
                    <div className="gp-autocomplete-dropdown" style={{ background: bg, borderColor: primary, borderRadius: radius }}>
                      {loadingGuests ? <div className="gp-autocomplete-empty">Loading guests…</div> : filteredResults.length === 0 ? <div className="gp-autocomplete-empty">No matching guests found</div> : filteredResults.map((g, i) => (
                        <button key={g.id} className={`gp-autocomplete-item ${i === highlightIndex ? 'highlighted' : ''}`} onClick={() => handleSelectGuest(g)} style={{ background: i === highlightIndex ? `${primary}15` : 'transparent' }}>
            ...
            {findView === 'table' && selectedGuest && (
              <div className="gp-table-info">
                ...
                <div className="gp-table-info-card" style={{ background: bg, borderColor: primary, borderRadius: radius }}>
                  <div className="gp-table-info-left"><p className="gp-table-info-name" style={{ color: text, fontSize: '22.5px' }}>{selectedGuest.name}</p></div>
                  <div className="gp-table-info-right"><span className="gp-table-info-badge" style={{ background: bg, borderColor: primary, color: primary, borderRadius: radius, fontSize: '18px' }}>{selectedGuest.table_name}</span></div>
                </div>
        ...
        {activeTab === 'layout' && (
          <div className="gp-layout-section">
            {venueImageUrl ? <div className="gp-layout-viewport" style={{ borderColor: primary, borderRadius: radius }} onClick={() => setLightboxOpen(true)}><img src={venueImageUrl} alt="Venue layout" className="gp-venue-img" style={{ borderRadius: radius }} draggable={false} /></div> : <div className="gp-no-layout" style={{ color: text }}><p>No venue layout available.</p></div>}
          </div>
        )}
      </div>
      {lightboxOpen && venueImageUrl && <VenueLightbox src={venueImageUrl} onClose={() => setLightboxOpen(false)} />}
    </div>
  )
}
...
function VenueLightbox({ src, onClose }: { src: string; onClose: () => void }) {
  ...
  useEffect(() => { const h = (e: KeyboardEvent) => { if (e.key === 'Escape') handleClose() }; document.addEventListener('keydown', h); return () => document.removeEventListener('keydown', h) }, [handleClose])
  ...
  return (
    <div className={`lightbox-overlay ${closing ? 'lightbox-closing' : ''}`} onClick={handleBackdropClick} style={{ touchAction: 'none' }}>
      <button className="lightbox-close" onClick={handleClose} aria-label="Close"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button>
      <div className="lightbox-content" ref={containerRef} onWheel={handleWheel} onDoubleClick={handleDoubleClick} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ cursor: isDragging ? 'grabbing' : zoom > 1 ? 'grab' : 'default' }}>
        <img src={src} alt="Venue layout" className="lightbox-img" style={{ transform: `translate(${panX}px, ${panY}px) scale(${zoom})`, transformOrigin: 'center', transition: isDragging ? 'none' : 'transform 0.15s ease-out' }} draggable={false} />
      </div>
    </div>
  )
}
