    ...
    <>
      ...
      <div className="page">
        ...
        <div style={{ marginBottom: 'var(--space-5)' }}>
          <button className="btn btn-primary" onClick={() => setShowCreateForm((s) => !s)}>{showCreateForm ? 'Cancel' : 'Create New Event'}</button>
        </div>
        {showCreateForm && (
            ...
            {events.map((event: Event) => (
              <div key={event.id} className="card card-sm event-card" onClick={() => navigate(`/events/${event.id}`)}>
                ...
                <div className="event-card-actions">
                  <button className="btn btn-primary btn-sm" onClick={(e) => { e.stopPropagation(); navigate(`/events/${event.id}`) }}>Manage</button>
