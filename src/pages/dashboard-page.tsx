...
export function DashboardPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const ev = await createEvent.mutateAsync({ name: name.trim(), slug })
      toast('Event created')
      setShowModal(false)
      setName('')
    ...
    <>
      ...
      <div className="page">
        <div className="page-header">
          ...
          <p className="page-subtitle">Manage your event seating plans</p>
        </div>
        {isLoading ? (
              ...
              <div key={ev.id} className="card event-card" onClick={() => navigate(`/events/${ev.id}`)}>
                ...
                <div className="event-card-footer">
                  ...
                  <div className="event-card-actions">
                    <button className="btn btn-ghost btn-sm" onClick={(e) => handleDelete(ev.id, ev.name, e)}>Delete</button>
                  </div>
                </div>
              </div>
            ))}
            <button className="new-event-btn" onClick={() => setShowModal(true)}>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate}>
              <div className="form-group">
                ...
                <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="My Wedding" autoFocus required />
              </div>
