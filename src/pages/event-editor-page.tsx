...
const LOGO_SIZES = [{ label: 'Small', value: 80 }, { label: 'Medium', value: 160 }, { label: 'Large', value: 280 }, { label: 'Extra Large', value: 420 }]

function TimeSelector({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const time24 = value || '09:00'; const [h, m] = time24.split(':'); let hour = parseInt(h, 10)
...
function FontDropdown({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return <select className="select font-dropdown" value={value} onChange={(e) => onChange(e.target.value)}>{FONTS.map((f) => <option key={f.name} value={f.name} style={{ fontFamily: getFontCss(f.name) }}>{f.name}</option>)}</select>
}
...
export function EventEditorPage() {
  ...
  const { data: guests } = useGuests(eventId ?? ''); const { data: tables } = useTables(eventId ?? '')
...
function DetailsTab({ event, settings, eventId, updateEvent, upsertSettings, toast }: any) {
  const [name, setName] = useState(event.name); const [subtitle, setSubtitle] = useState(settings?.event_subtitle ?? '')
  const [date, setDate] = useState(event.date ?? ''); const [time, setTime] = useState(event.time ?? '09:00')
  const [venue, setVenue] = useState(event.venue ?? ''); const [detailsDirty, setDetailsDirty] = useState(false)
  ...
  const [logoUploading, setLogoUploading] = useState(false); const [logoDragOver, setLogoDragOver] = useState(false)
  ...
  useEffect(() => { if (!settings) return; setTitleFont(settings.font_title_family ?? 'Inter'); setTitleSize(settings.font_title_size ?? 32); setTitleColor(settings.font_title_color ?? '#0f172a'); setSubtitleFont(settings.font_subtitle_family ?? 'Inter'); setSubtitleSize(settings.font_subtitle_size ?? 16); setSubtitleColor(settings.font_subtitle_color ?? '#64748b'); setDatetimeFont(settings.font_datetime_family ?? 'Inter'); setDatetimeSize(settings.font_datetime_size ?? 14); setDatetimeColor(settings.font_datetime_color ?? '#64748b'); setVenueFont(settings.font_venue_family ?? 'Inter'); setVenueSize(settings.font_venue_size ?? 14); setVenueColor(settings.font_venue_color ?? '#64748b'); setSubtitle(settings.event_subtitle ?? ''); setLogoUrl(settings.logo_url ?? null); setLogoSize(settings.logo_size ?? 80) }, [settings])
  useEffect(() => { setDetailsDirty(name !== event.name || subtitle !== (settings?.event_subtitle ?? '') || date !== (event.date ?? '') || time !== (event.time ?? '09:00') || venue !== (event.venue ?? '') || logoUrl !== (settings?.logo_url ?? null) || logoSize !== (settings?.logo_size ?? 80)) }, [name, subtitle, date, time, venue, logoUrl, logoSize, event, settings])
  const allFonts = [titleFont, subtitleFont, datetimeFont, venueFont]
  useEffect(() => { loadGoogleFonts(allFonts) }, allFonts)
  useEffect(() => { if (!settings) { setTypoDirty(false); return }; setTypoDirty(titleFont !== (settings.font_title_family ?? 'Inter') || titleSize !== (settings.font_title_size ?? 32) || titleColor !== (settings.font_title_color ?? '#0f172a') || subtitleFont !== (settings.font_subtitle_family ?? 'Inter') || subtitleSize !== (settings.font_subtitle_size ?? 16) || subtitleColor !== (settings.font_subtitle_color ?? '#64748b') || datetimeFont !== (settings.font_datetime_family ?? 'Inter') || datetimeSize !== (settings.font_datetime_size ?? 14) || datetimeColor !== (settings.font_datetime_color ?? '#64748b') || venueFont !== (settings.font_venue_family ?? 'Inter') || venueSize !== (settings.font_venue_size ?? 14) || venueColor !== (settings.font_venue_color ?? '#64748b')) }, [titleFont, titleSize, titleColor, subtitleFont, subtitleSize, subtitleColor, datetimeFont, datetimeSize, datetimeColor, venueFont, venueSize, venueColor, settings])
  const handleLogoFile = async (file: File) => { if (!file.type.match(/image\/(png|jpe?g|svg\+xml|webp)/)) { toast('Please use PNG, JPG, SVG, or WebP', 'error'); return }; setLogoUploading(true); try { setLogoUrl(await uploadEventImage(eventId, file, 'logo')) } catch (err) { toast(err instanceof Error ? err.message : 'Failed to upload logo', 'error') } finally { setLogoUploading(false) } }
  const handleSaveDetails = async () => { try { await updateEvent.mutateAsync({ id: eventId, name, slug: event.slug, date: date || null, time: time || null, venue: venue || null }); await upsertSettings.mutateAsync({ event_id: eventId, event_subtitle: subtitle || null, logo_url: logoUrl, logo_size: logoSize }); toast('Event details saved'); setDetailsDirty(false) } catch (err) { toast(err instanceof Error ? err.message : 'Failed to save', 'error') } }
          ...
          <div className="logo-section" style={{ marginTop: 'var(--space-5)' }}>
            ...
            {logoUrl ? (
              <div className="logo-preview-area">
                ...
                <div className="logo-actions"><button className="btn btn-ghost btn-sm" onClick={() => logoFileRef.current?.click()} disabled={logoUploading}>Replace</button><button className="btn btn-ghost btn-sm" onClick={() => setLogoUrl(null)} disabled={logoUploading}>Remove</button></div>
              </div>
            ) : (
              <div className={`dropzone ${logoDragOver ? 'dropzone-active' : ''}`} onDragOver={(e) => { e.preventDefault(); setLogoDragOver(true) }} onDragLeave={() => setLogoDragOver(false)} onDrop={(e) => { e.preventDefault(); setLogoDragOver(false); if (e.dataTransfer.files[0]) handleLogoFile(e.dataTransfer.files[0]) }} onClick={() => logoFileRef.current?.click()}>
                <p className="dropzone-text">{logoUploading ? 'Uploading…' : 'Drop logo here or click to browse'}</p><p className="dropzone-hint">PNG, JPG, SVG, or WebP</p>
              </div>
            )}
...
function GuestsTab({ eventId, guests, tables, toast, confirm }: any) {
  const [search, setSearch] = useState(''); const [sortBy, setSortBy] = useState<'name' | 'table'>('name'); const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc')
  const [filterTable, setFilterTable] = useState(''); const [editingId, setEditingId] = useState<string | null>(null); const [editName, setEditName] = useState(''); const [editTable, setEditTable] = useState('')
  ...
  const handleSortClick = (field: 'name' | 'table') => { if (sortBy === field) setSortDir((prev) => prev === 'asc' ? 'desc' : 'asc'); else { setSortBy(field); setSortDir('asc') } }
  ...
  const handleBulkAdd = async () => { const lines = bulkText.split(/\r?\n/).map((l) => l.trim()).filter(Boolean); if (lines.length === 0) { toast('Enter at least one guest name', 'error'); return }; try { await bulkCreateGuests.mutateAsync({ event_id: eventId, guests: lines.map((n: string) => ({ name: n, event_id: eventId, table_id: bulkTable || null })) }); toast(`Added ${lines.length} guests`); setBulkText('') } catch (err) { toast(classifyError(err), 'error') } }
  ...
  const handleConfirmImport = async () => { const valid = parsedGuests.filter((g) => g.name.trim()); if (valid.length === 0) return; setImporting(true); try { const gi: GuestInput[] = valid.map((g) => ({ name: g.name.trim(), event_id: eventId, table_id: g.tableName.trim() ? matchTableByName(g.tableName, tables) : null })); await bulkCreateGuests.mutateAsync({ event_id: eventId, guests: gi }); toast(`Imported ${valid.length} guests`); setParsedGuests([]); setImportFile(null) } catch (err) { toast(classifyError(err), 'error') } finally { setImporting(false) } }
  return (
    <div className="two-col">
      <div className="card">
        ...
        <div className="guest-list">
          {filtered.length === 0 ? <div className="empty-state" style={{ padding: 'var(--space-7)' }}><p>{search || filterTable ? 'No guests match your filters' : 'No guests yet. Add some from the panel on the right.'}</p></div> : filtered.map((g: any) => (
            <div key={g.id} className="guest-row">
              {editingId === g.id ? (
                <div className="guest-row-edit"><input className="input" value={editName} onChange={(e) => setEditName(e.target.value)} autoFocus /><select className="select" value={editTable} onChange={(e) => setEditTable(e.target.value)}><option value="">No Table</option>{tables.map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}</select><button className="btn btn-primary btn-sm" onClick={handleSaveEdit}>Save</button><button className="btn btn-ghost btn-sm" onClick={() => setEditingId(null)}>Cancel</button></div>
              ) : (<><span className="guest-name">{g.name}</span><span className="guest-table">{g.table_id ? tableMap.get(g.table_id)?.name ?? 'Unknown' : 'No table'}</span><div className="guest-row-actions"><button className="btn btn-ghost btn-sm" onClick={() => handleEdit(g)}>Edit</button><button className="btn btn-ghost btn-sm" onClick={() => handleDelete(g.id, g.name)}>Delete</button></div></>)}
            </div>
          ))}
        </div>
      </div>
      <div className="guest-mgmt-panel">
        <div className="card card-sm section">
          ...
          <textarea className="textarea bulk-textarea" placeholder={'John Tan\nSarah Lee\nAhmad Bin Ali'} value={bulkText} onChange={(e) => setBulkText(e.target.value)} rows={10} />
        ...
        <div className="card card-sm section">
          ...
          {!parsedGuests.length ? (
            <div className={`dropzone ${dragOver ? 'dropzone-active' : ''}`} onDragOver={(e) => { e.preventDefault(); setDragOver(true) }} onDragLeave={() => setDragOver(false)} onDrop={(e) => { e.preventDefault(); setDragOver(false); e.dataTransfer.files[0] && handleFileSelect(e.dataTransfer.files[0]) }} onClick={() => fileInputRef.current?.click()}>
              <p className="dropzone-text">{importing ? 'Parsing…' : 'Drop file here or click to browse'}</p><p className="dropzone-hint">Supports CSV, XLSX, XLS, PDF</p>
              <input ref={fileInputRef} type="file" accept=".csv,.xlsx,.xls,.pdf" style={{ display: 'none' }} onChange={(e) => e.target.files?.[0] && handleFileSelect(e.target.files[0])} />
            </div>
          ) : (
            <div className="import-review">
              ...
              <div className="import-review-list">{parsedGuests.map((g, i) => <div key={i} className="import-review-row"><input className="input" value={g.name} onChange={(e) => { const u = [...parsedGuests]; u[i].name = e.target.value; setParsedGuests(u) }} /><input className="input" placeholder="Table" value={g.tableName} onChange={(e) => { const u = [...parsedGuests]; u[i].tableName = e.target.value; setParsedGuests(u) }} /><button className="btn btn-ghost btn-sm" onClick={() => setParsedGuests(parsedGuests.filter((_, idx) => idx !== i))}>Remove</button></div>)}</div>
...
function TablesTab({ eventId, tables, guests, toast, confirm }: any) {
  const [mode, setMode] = useState<'sequential' | 'custom'>('sequential'); const [prefix, setPrefix] = useState('Table'); const [startNum, setStartNum] = useState(1); const [count, setCount] = useState(10); const [seats, setSeats] = useState(8)
  ...
  const handleSequentialCreate = async () => { const newTables = Array.from({ length: count }, (_, i) => ({ name: `${prefix} ${startNum + i}`, number: startNum + i, capacity: seats })); try { await bulkCreateTables.mutateAsync({ event_id: eventId, tables: newTables }); toast(`Created ${count} tables`) } catch (err) { toast(err instanceof Error ? err.message : 'Failed to create tables', 'error') } }
  const handleCustomCreate = async () => { const valid = customRows.filter((r) => r.name.trim()); if (valid.length === 0) { toast('Enter at least one table name', 'error'); return }; const names = valid.map((r) => r.name.trim()); const dupes = names.filter((n, i) => names.indexOf(n) !== i); if (dupes.length) { toast(`Duplicate table names: ${dupes.join(', ')}`, 'error'); return }; const conflicts = names.filter((n) => tables.map((t: any) => t.name.toLowerCase()).includes(n.toLowerCase())); if (conflicts.length) { toast(`Tables already exist: ${conflicts.join(', ')}`, 'error'); return }; try { await bulkCreateTables.mutateAsync({ event_id: eventId, tables: valid.map((r, i) => ({ name: r.name.trim(), number: tables.length + i + 1, capacity: r.capacity })) }); toast(`Created ${valid.length} tables`); setCustomRows([{ name: '', capacity: 8 }]) } catch (err) { toast(err instanceof Error ? err.message : 'Failed to create tables', 'error') } }
  ...
  return (
    <div className="section">
      <div className="card section">
        ...
        <div className="mode-toggle"><button className={`btn btn-sm ${mode === 'sequential' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => setMode('sequential')}>Sequential Numbering</button><button className={`btn btn-sm ${mode === 'custom' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => setMode('custom')}>Custom Names</button></div>
        {mode === 'sequential' ? (
          <div className="bulk-form">
            <div className="form-row"><div className="form-group"><label className="form-label">Prefix</label><input className="input" value={prefix} onChange={(e) => setPrefix(e.target.value)} /></div><div className="form-group"><label className="form-label">Start Number</label><input type="number" className="input" value={startNum} min={1} onChange={(e) => setStartNum(Number(e.target.value))} /></div><div className="form-group"><label className="form-label">Count</label><input type="number" className="input" value={count} min={1} max={100} onChange={(e) => setCount(Number(e.target.value))} /></div><div className="form-group"><label className="form-label">Seats per Table</label><input type="number" className="input" value={seats} min={1} onChange={(e) => setSeats(Number(e.target.value))} /></div></div>
            <div className="bulk-preview"><span className="form-hint">Will create: {prefix} {startNum}, {prefix} {startNum + 1}, … {prefix} {startNum + count - 1}</span></div>
            <button className="btn btn-primary" onClick={handleSequentialCreate} disabled={bulkCreateTables.isPending}>{bulkCreateTables.isPending ? 'Creating…' : `Create ${count} Tables`}</button>
          </div>
        ) : (
          <div className="bulk-form">
            <div className="custom-rows">{customRows.map((row, i) => <div key={i} className="custom-row"><input className="input" placeholder="Table name" value={row.name} onChange={(e) => { const u = [...customRows]; u[i].name = e.target.value; setCustomRows(u) }} /><input type="number" className="input" placeholder="Capacity" value={row.capacity} min={1} onChange={(e) => { const u = [...customRows]; u[i].capacity = Number(e.target.value); setCustomRows(u) }} style={{ maxWidth: '120px' }} /><button className="btn btn-ghost btn-sm" onClick={() => setCustomRows(customRows.filter((_, idx) => idx !== i))}>Remove</button></div>)}</div>
            <div className="manual-actions"><button className="btn btn-ghost btn-sm" onClick={() => setCustomRows([...customRows, { name: '', capacity: 8 }])}>+ Add Row</button><button className="btn btn-primary btn-sm" onClick={handleCustomCreate} disabled={bulkCreateTables.isPending}>{bulkCreateTables.isPending ? 'Creating…' : 'Create Tables'}</button></div>
          </div>
        )}
      </div>
...
function LayoutTab({ eventId, settings, upsertSettings, toast }: any) {
  const [image, setImage] = useState<string | null>(settings?.venue_image_url ?? null); const [dragOver, setDragOver] = useState(false); const [saving, setSaving] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)
  ...
  return (
    <div className="section">
      <div className="card">
        <div className="card-header"><h3 className="card-title">Venue Layout</h3><p className="card-subtitle">Upload a floor plan or seating chart image</p></div>
        {image ? (
          <div className="venue-preview"><img src={image} alt="Venue layout" className="venue-preview-img" /><div className="venue-preview-actions"><button className="btn btn-ghost btn-sm" onClick={() => fileInputRef.current?.click()}>Replace</button><button className="btn btn-ghost btn-sm" onClick={() => setImage(null)}>Remove</button></div></div>
        ) : (
          <div className={`dropzone dropzone-lg ${dragOver ? 'dropzone-active' : ''}`} onDragOver={(e) => { e.preventDefault(); setDragOver(true) }} onDragLeave={() => setDragOver(false)} onDrop={(e) => { e.preventDefault(); setDragOver(false); e.dataTransfer.files[0] && handleFile(e.dataTransfer.files[0]) }} onClick={() => fileInputRef.current?.click()}>
            <p className="dropzone-text">Drop image here or click to browse</p><p className="dropzone-hint">PNG, JPG, SVG, or WebP</p>
          </div>
        )}
        <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" style={{ display: 'none' }} onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
        {image && <div style={{ marginTop: 'var(--space-5)' }}><button className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Saving…' : 'Save Layout'}</button></div>}
      </div>
    </div>
  )
}
...
function ThemeTab({ eventId, settings, upsertSettings, toast }: any) {
  const [primary, setPrimary] = useState(settings?.color_primary ?? '#0f766e'); const [background, setBackground] = useState(settings?.color_background ?? '#f8fafc')
  const [card, setCard] = useState(settings?.color_card ?? '#ffffff'); const [text, setText] = useState(settings?.color_text ?? '#0f172a'); const [header, setHeader] = useState(settings?.color_header ?? '#ffffff')
  const [radius, setRadius] = useState(settings?.border_radius ?? 16); const [logoSize, setLogoSize] = useState(settings?.logo_size ?? 80); const [logoRounded, setLogoRounded] = useState(settings?.logo_rounded ?? false)
  ...
  return (
    <div className="section">
      <div className="card section">
        <div className="card-header"><h3 className="card-title">Colors</h3><p className="card-subtitle">Customise your guest website color scheme</p></div>
        <div className="grid grid-3">{colorFields.map((f) => <div key={f.label} className="form-group"><label className="form-label">{f.label}</label><div className="color-field"><input type="color" className="color-picker" value={f.value} onChange={(e) => f.setter(e.target.value)} /><input className="input" value={f.value} onChange={(e) => f.setter(e.target.value)} style={{ flex: 1 }} /></div></div>)}</div>
      </div>
      <div className="card section">
        <div className="card-header"><h3 className="card-title">Layout</h3></div>
        <div className="form-row"><div className="form-group"><label className="form-label">Border Radius ({radius}px)</label><input type="range" className="range" min={0} max={32} value={radius} onChange={(e) => setRadius(Number(e.target.value))} /></div><div className="form-group"><label className="form-label">Logo Size ({logoSize}px)</label><input type="range" className="range" min={40} max={500} step={10} value={logoSize} onChange={(e) => setLogoSize(Number(e.target.value))} /></div></div>
        <div className="form-group" style={{ marginTop: 'var(--space-4)' }}><label className="checkbox-label"><input type="checkbox" checked={logoRounded} onChange={(e) => setLogoRounded(e.target.checked)} /> Rounded logo corners</label></div>
      </div>
      <button className="btn btn-primary" onClick={handleSave} disabled={!dirty || saving}>{saving ? 'Saving…' : 'Save Theme'}</button>
    </div>
  )
}
...
function ShareTab({ event, eventId, upsertSettings, toast }: any) {
  ...
  return (
    <div className="section">
      <div className="card section"><div className="card-header"><h3 className="card-title">Guest Website URL</h3><p className="card-subtitle">Share this link with your guests</p></div><div className="share-url"><input className="input" value={guestUrl} readOnly /><button className="btn btn-ghost" onClick={() => handleCopy(guestUrl)}>Copy</button><button className="btn btn-primary" onClick={() => window.open(guestUrl, '_blank')}>Open</button></div></div>
      <div className="card section">
        ...
        <div className="form-row"><div className="form-group"><label className="form-label">Slug</label><input className="input" value={slug} onChange={(e) => { setSlug(e.target.value); setSlugCheck('') }} placeholder="my-event" /></div><div className="form-group" style={{ flex: '0 0 auto', alignSelf: 'flex-end' }}><button className="btn btn-ghost" onClick={async () => { if (!slug.trim()) return; setSlugCheck(slug.trim()); await refetch() }} disabled={isFetching}>Check</button></div></div>
        {slugResult && slugCheck && <p className={`form-hint ${slugResult.available ? 'hint-success' : 'hint-error'}`}>{slugResult.available ? 'URL is available!' : 'URL is already taken'}</p>}
        <div style={{ marginTop: 'var(--space-4)' }}><button className="btn btn-primary" onClick={async () => { if (!slug.trim()) return; try { await upsertSettings.mutateAsync({ event_id: eventId }); toast('URL updated') } catch (err) { toast(err instanceof Error ? err.message : 'Failed to update URL', 'error') } }} disabled={!slug.trim()}>Save URL</button></div>
      </div>
      <div className="card section"><div className="card-header"><h3 className="card-title">QR Code</h3><p className="card-subtitle">Download a QR code for printed materials</p></div><div className="qr-section">{qrUrl && <img src={qrUrl} alt="QR Code" className="qr-preview" />}<div className="qr-actions"><button className="btn btn-ghost" onClick={() => handleDownloadQR('png')}>Download PNG</button><button className="btn btn-ghost" onClick={() => handleDownloadQR('svg')}>Download SVG</button></div></div></div>
    </div>
  )
}
