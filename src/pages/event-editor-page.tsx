...
function TimeSelector({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const handleHour = (n: number) => {
    let h24 = n
    if (period === 'PM' && n !== 12) h24 += 12
    if (period === 'AM' && n === 12) h24 = 0
    onChange(`${String(h24).padStart(2, '0')}:${m}`)
  }
  const handleMinute = (n: string) => {
    let h24 = hour
    if (period === 'PM' && hour !== 12) h24 = hour + 12
    if (period === 'AM' && hour === 12) h24 = 0
    onChange(`${String(h24).padStart(2, '0')}:${n}`)
  }
  const handlePeriod = (p: string) => {
    let h24 = hour
    if (p === 'PM' && hour !== 12) h24 = hour + 12
    if (p === 'AM' && hour === 12) h24 = 0
    if (p === 'PM' && hour === 12) h24 = 12
    onChange(`${String(h24).padStart(2, '0')}:${m}`)
  }
  return (
...
function TypoCard({ label, font, size, color, onFont, onSize, onColor, previewText, previewStyle }: {
...
function DetailsTab({ event, settings, eventId, updateEvent, upsertSettings, toast }: any) {
  ...
  useEffect(() => {
    ...
    setTypoDirty(
      ...
      titleColor !== (settings.font_title_color ?? '#0f172a') ||
      subtitleFont !== (settings.font_subtitle_family ?? 'Inter') ||
      subtitleSize !== (settings.font_subtitle_size ?? 16) ||
      subtitleColor !== (settings.font_subtitle_color ?? '#64748b') ||
      datetimeFont !== (settings.font_datetime_family ?? 'Inter') ||
      datetimeSize !== (settings.font_datetime_size ?? 14) ||
      datetimeColor !== (settings.font_datetime_color ?? '#64748b') ||
      venueFont !== (settings.font_venue_family ?? 'Inter') ||
      venueSize !== (settings.font_venue_size ?? 14) ||
      venueColor !== (settings.font_venue_color ?? '#64748b')
    )
...
function GuestsTab({ eventId, guests, tables, toast, confirm }: any) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateGuest.mutateAsync({ id: editingId, event_id: eventId, name: editName.trim(), table_id: editTable || null })
      toast('Guest updated')
      setEditingId(null)
  ...
  const handleBulkAdd = async () => {
    ...
    try {
      const guestsInput: GuestInput[] = lines.map((n: string) => ({ name: n, event_id: eventId, table_id: bulkTable || null }))
    ...
    <div className="two-col">
      <div className="card">
        ...
        <div className="sort-toggle">
          <button className={`sort-toggle-btn ${sortBy === 'name' ? 'active' : ''}`} onClick={() => handleSortClick('name')}>Name {sortBy === 'name' && (sortDir === 'asc' ? '↑' : '↓')}</button>
          <button className={`sort-toggle-btn ${sortBy === 'table' ? 'active' : ''}`} onClick={() => handleSortClick('table')}>Table {sortBy === 'table' && (sortDir === 'asc' ? '↑' : '↓')}</button>
        </div>
...
function TablesTab({ eventId, tables, guests, toast, confirm }: any) {
  ...
  const handleSequentialCreate = async () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      newTables.push({ name: `${prefix} ${num}`, number: num, capacity: seats })
    }
  ...
  const handleCustomCreate = async () => {
    ...
    const existingNames = tables.map((t: any) => t.name.toLowerCase())
    ...
    const newTables = valid.map((r, i) => ({ name: r.name.trim(), number: tables.length + i + 1, capacity: r.capacity }))
...
function ThemeTab({ eventId, settings, upsertSettings, toast }: any) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await upsertSettings.mutateAsync({ event_id: eventId, color_primary: primary, color_background: background, color_card: card, color_text: text, color_header: header, border_radius: radius, logo_size: logoSize, logo_rounded: logoRounded })
      toast('Theme saved'); setDirty(false)
    } catch (err) { toast(err instanceof Error ? err.message : 'Failed to save', 'error') }
    finally { setSaving(false) }
  }
  ...
  return (
    <div className="section">
      ...
      <div className="card section">
        ...
        <div className="form-row">
          <div className="form-group"><label className="form-label">Slug</label><input className="input" value={slug} onChange={(e) => { setSlug(e.target.value); setSlugCheck('') }} placeholder="my-event" /></div>
          <div className="form-group" style={{ flex: '0 0 auto', alignSelf: 'flex-end' }}><button className="btn btn-ghost" onClick={handleCheckSlug} disabled={isFetching}>Check</button></div>
        </div>
