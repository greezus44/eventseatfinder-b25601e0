...
export function FindYourSeatPage() {
  ...
  const handleSearch = async () => {
    ...
    const { data: guests } = await supabase
      .from('guests')
      .select('name, table_id, tables!inner(name, number)')
      .ilike('name', `%${search.trim()}%`)
      .eq('event_id', settingsData.event_id)

    if (guests) {
      setResults(
        guests.map((g: any) => ({
          name: g.name,
          table_name: g.tables?.name ?? 'Unassigned',
          table_number: g.tables?.number ?? 0,
        }))
      )
    }
  }
  ...
  return (
    <div className="gp-page" style={{ background: bg, color: text }}>
      <div className="gp-container">
        ...
        {settings.logo_url && (
          <div className="gp-logo-wrapper">
            <img
              ...
              className="gp-logo"
              style={{
                width: `${Math.min(settings.logo_size ?? 80, 200)}px`,
                height: 'auto',
        ...
        {/* Event Name */}
        <h1 className="gp-title" style={{ fontSize: '1.75rem', fontWeight: 700 }}>
          {event.name}
        </h1>
        ...
        <div className="gp-tabs">
          <button
            className={`gp-tab ${activeTab === 'find' ? 'active' : ''}`}
            onClick={() => setActiveTab('find')}
            style={activeTab === 'find' ? { background: primary, color: '#fff' } : {}}
          >
            Find Seat
          </button>
          <button
            className={`gp-tab ${activeTab === 'layout' ? 'active' : ''}`}
            onClick={() => setActiveTab('layout')}
            style={activeTab === 'layout' ? { background: primary, color: '#fff' } : {}}
          >
            Venue Layout
          </button>
        </div>
                  ...
                  results.map((r, i) => (
                    <div key={i} className="gp-result-card" style={{ background: card, borderRadius: radius }}>
                      ...
                      <p className="gp-result-table">
                        ...
                        <span className="gp-result-table-value" style={{ color: primary }}>
                          {r.table_name}
                        </span>
                      </p>
                    </div>
        ...
        {activeTab === 'layout' && (
          <div className="gp-layout-section">
            {settings.venue_image_url ? (
              ...
              <div className="gp-no-layout">
                <p>No venue layout available.</p>
              </div>
            )}
          </div>
        )}
