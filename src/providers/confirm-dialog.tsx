...
export function ConfirmDialogProvider({ children }: { children: ReactNode }) {
  ...
  return <Ctx.Provider value={{ confirm }}>{children}{d.open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"><div className="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">{d.title && <h2 className="mb-2 text-lg font-semibold text-slate-900">{d.title}</h2>}<p className="mb-6 text-sm text-slate-600">{d.message}</p><div className="flex justify-end gap-3"><button type="button" onClick={cancelBtn} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">{d.cancelText ?? 'Cancel'}</button><button type="button" onClick={confirmBtn} className="rounded-md bg-teal-700 px-4 py-2 text-sm font-medium text-white hover:bg-teal-800">{d.confirmText ?? 'Confirm'}</button></div></div></div>}</Ctx.Provider>
}
