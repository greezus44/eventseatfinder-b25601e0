...
export function useCreateGuest() {
  ...
  return useMutation({ mutationFn: async (input: GuestInput) => {
    ...
    if (error) throw error; return data as Guest
  }, onSuccess: (c) => qc.invalidateQueries({ queryKey: [GUESTS_KEY, c.event_id] }) })
}
export function useUpdateGuest() {
  ...
  return useMutation({ mutationFn: async ({ id, ...input }: { id: string } & GuestInput) => {
    ...
    if (error) throw error; return data as Guest
  }, onSuccess: (u) => qc.invalidateQueries({ queryKey: [GUESTS_KEY, u.event_id] }) })
}
...
export function useBulkCreateGuests() {
  ...
  return useMutation({ mutationFn: async ({ event_id, guests }: { event_id: string; guests: GuestInput[] }) => {
    const rows = guests.map((g) => ({ event_id, name: g.name, table_id: g.table_id ?? null }))
    const inserted: Guest[] = []
    for (let i = 0; i < rows.length; i += 500) {
      const { data, error } = await supabase.from('guests').insert(rows.slice(i, i + 500)).select()
      if (error) throw error; if (data) inserted.push(...(data as Guest[]))
    }
    return inserted
  }, onSuccess: (_d, v) => qc.invalidateQueries({ queryKey: [GUESTS_KEY, v.event_id] }) })
}
