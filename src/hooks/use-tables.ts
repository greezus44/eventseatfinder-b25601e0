...
export function useBulkCreateTables() {
  ...
  return useMutation({
    mutationFn: async ({ event_id, tables }: { event_id: string; tables: Omit<TableInput, 'event_id'>[] }) => {
      const rows = tables.map((t) => ({ ...t, event_id }))
      const { data, error } = await supabase.from('tables').insert(rows).select()
      if (error) throw error
      return { event_id, data }
    },
    onSuccess: (result) => { qc.invalidateQueries({ queryKey: ['tables', result.event_id] }) },
  })
}
...
export function useDeleteTable() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { data: table } = await supabase.from('tables').select('event_id').eq('id', id).maybeSingle()
      await supabase.from('guests').update({ table_id: null }).eq('table_id', id)
      const { error } = await supabase.from('tables').delete().eq('id', id)
      if (error) throw error
      return table?.event_id as string | undefined
    },
    onSuccess: (eventId) => {
      if (eventId) {
